...
public class PK2Archive : IDisposable
{
    ...
    public PK2File GetFile(string path, bool absolutePath = false)
    {
        ...
        return absolutePath
            ...
            : _files.Where(file => file.Name.ToLowerInvariant() == path.ToLowerInvariant())
                .OrderByDescending(file => file.CreateTime).FirstOrDefault();
    }
    ...
    public PK2Directory[] GetDirectories(string path, bool recursive)
    {
        ...
        return recursive
            ? _directories.Where(directory => directory.Path.StartsWith(path)).ToArray()
            : _directories.Where(directory => directory.Path == path).ToArray();
    }
    ...
    public PK2File[] GetFiles(string path, bool recursive)
    {
        ...
        return recursive
            ? _files.Where(file => file.Path.StartsWith(path)).ToArray()
            : _files.Where(file => file.Path == path).ToArray();
    }
}