...
{
    public NavMeshEdgeInternal(NavMesh mesh, int index) : base(mesh, index)
    {
    }
    ...
    public override void Link()
    {
        ...
        if (/*!this.IsBlocked &&*/ this.DstCellIndex > 0)
        {
            this.DstCell = this.NavMesh.GetNavMeshCell(this.DstCellIndex);
            if (this.DstCell == null)
                Debug.WriteLine($"Couldn't find DstCellIndex {this.DstCellIndex} on {this.NavMesh} when linking {this}");
